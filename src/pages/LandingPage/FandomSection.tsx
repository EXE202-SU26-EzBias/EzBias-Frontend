  ...
  return (
    <section className="mx-auto max-w-[1400px] px-6 pb-16 pt-8" aria-label="Shop by Fandom">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {fandoms.length === 0 ? (
          <p className="col-span-full py-12 text-center text-sm text-[#737373]">Fandoms coming soon.</p>
        ) : (
          fandoms.map((f) => (
            <Link
              key={f.name}
            ...
            >
              <div
                className="fandom-card-bg relative flex aspect-[4/3] items-center justify-center overflow-hidden"
                style={{ '--fandom-accent': f.accent } as React.CSSProperties}
              >
