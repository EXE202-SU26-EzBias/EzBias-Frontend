...
const PayoutRejectModal = React.memo(function PayoutRejectModal({
  payout,
  isPending,
  onConfirm,
  onClose,
  ...
  const isInvalid = reason.trim().length < MIN_REASON_LENGTH;
    ...
    >
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md">
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#e6e6e6]">
          <h2 className="text-[17px] font-bold text-[#121212]">Reject Payout</h2>
          <button
            type="button"
            onClick={onClose}
            disabled={isPending}
        ...
        <div className="px-6 py-5 flex flex-col gap-4">
          <div className="bg-[rgba(239,68,68,0.06)] rounded-xl border border-[rgba(239,68,68,0.15)] px-4 py-3 flex flex-col gap-1">
            <span className="text-[11px] font-semibold text-[#737373] uppercase tracking-wide">Rejecting payout</span>
            <span className="text-[20px] font-bold text-[#dc2626]">{formatCurrency(payout.amount)}</span>
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <textarea
              ...
              disabled={isPending}
              className={`w-full rounded-lg border px-3 py-2 text-[13px] text-[#121212] resize-none focus:outline-none transition-colors disabled:opacity-50 ${
                touched && isInvalid
                  ? 'border-[#ef4343] focus:border-[#ef4343]'
                  : 'border-[#e6e6e6] focus:border-[#ad93e6]'
              }`}
            />
            {touched && isInvalid && (
              <p className="text-[11px] text-[#ef4343]">
        ...
        <div className="flex items-center justify-end gap-2 px-6 py-4 border-t border-[#e6e6e6] bg-[#f9fafb] rounded-b-2xl">
          ...
          <button
            ...
            disabled={isPending}
            className="h-9 px-4 rounded-lg bg-red-600 hover:bg-red-700 text-white text-[13px] font-semibold transition-colors disabled:opacity-60 flex items-center gap-2"
          >
            {isPending && (
